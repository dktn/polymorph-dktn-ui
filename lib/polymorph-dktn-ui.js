...
export default {
    ...
    treeListAddOpen: function(event) {
        // if (event.item instanceof TextEditor && this.treeView) {
            // var textEditor = event.item;
        if (this.treeView) {
            var textEditor = event.textEditor;
            var filePath = textEditor.getPath();
            var entry = this.treeView.entryForPath(filePath);
            if (entry) {
                entry.classList.add('open');
            } else {
                console.log("Adding pane: Not found entry for ", filePath);
            }
        } else {
            console.log("Adding pane: Not found tree view");
        }
    },

    treeListAddOpenForCurrent: function(event) {
        var textEditor = atom.workspace.getActiveTextEditor();
        if (this.treeView && textEditor instanceof TextEditor) {
            var filePath = textEditor.getPath();
            var entry = this.treeView.entryForPath(filePath);
            if (entry) {
                entry.classList.add('open');
            } else {
                console.log("Refreshing pane: Not found entry for ", filePath);
            }
        } else {
            console.log("Refreshing pane: Not found tree view");
        }
    },
    ...
    treeListRemoveOpen: function(event){
        if (this.treeView && event.item instanceof TextEditor) {
            ...
            if (entry) {
                ...
                console.log("Removing pane: Not found entry for ", filePath);
            }
        }
    },
